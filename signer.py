...
def load_pfx(file_path, password, txn_id, webhook_url):
    ...
    try:
        ...
        log_transaction(txn_id, "failure", "Invalid password for the PFX file.", webhook_url)
        raise ValueError("Invalid password for the PFX file.")
    
    except FileNotFoundError:
        log_transaction(txn_id, "failure", "No such PFX Found. Please Upload with using /upload", webhook_url)
        raise ValueError("No such PFX Found. Please Upload with using /upload")
    
    except Exception as e:
        log_transaction(txn_id, "failure", f"Error loading PFX file: {str(e)}", webhook_url)
        raise ValueError(f"Error loading PFX file: {str(e)}")

# Function to validate PFX certificate file extension
def validate_args(pfx_certificate, password, txn_id, webhook_url):
    ...
    if not IS_PFX(pfx_certificate):
        log_transaction(txn_id, "failure", "Not a proper pfx file with .pfx or .PFX extension", webhook_url)
    ...
    try:
        ...
        if issuer_pem.startswith("-----BEGIN CERTIFICATE-----"):
            try:
                ...
                log_transaction(txn_id, "failure", f'Error loading PEM issuer cert: {str(pem_e)}', webhook_url)
        ...
        log_transaction(txn_id, "failure", f'Error fetching or parsing issuer certificate: {str(e)}', webhook_url)
        print(f"Error fetching or parsing issuer certificate: {str(e)}")
        raise

def get_ocsp_cert_status(ocsp_server, cert, issuer_cert, txn_id, webhook_url):
        ...
        try:
            ...
            if ocsp_decoded.response_status == ocsp.OCSPResponseStatus.SUCCESSFUL:
                ...
                if certificate_status == ocsp.OCSPCertStatus.REVOKED:
                    log_transaction(txn_id, "failure", 'The certificate is revoked From OCSP. Signing aborted. Please try another Valid certificate for signing.', webhook_url)
                ...
                log_transaction(txn_id, "failure", f'Decoding OCSP response failed: {ocsp_decoded.response_status}', webhook_url)
                raise Exception(f'Decoding OCSP response failed: {ocsp_decoded.response_status}')
        except Exception as e:
          
            raise
    else:
        log_transaction(txn_id, "failure", f'Fetching OCSP cert status failed with response status: {response.status_code}', webhook_url)
        raise Exception(f'Fetching OCSP cert status failed with response status: {response.status_code}')
    

def fetch_crl(certificate, txn_id, webhook_url):
    ...
    try:
        ...
        if not crl_urls:
            log_transaction(txn_id, "failure", 'No CRL Distribution Points found in the certificate.', webhook_url)
        ...
        log_transaction(txn_id, "failure", f'Error fetching CRL: {e}', webhook_url)
...
def check_certificate_expiry(cert, txn_id, webhook_url):
    ...
    if cert_expiry_ist < current_time_ist:
        log_transaction(txn_id, "failure", 'The certificate is Expired. Signing aborted. Please try another valid certificate for signing.', webhook_url)
        raise Exception("The certificate is Expired. Signing aborted. Please try another valid certificate for signing.") 
    

def validate_key_usage(certificate, txn_id, webhook_url):
    """
    ...
    try:
        ...
        if not key_usage.digital_signature:
            log_transaction(txn_id, "failure", 'The certificate is Not Digital Signature key_usage. Signing aborted. Please try another valid certificate for signing.', webhook_url)
        ...
        log_transaction(txn_id, "failure", f'Error validating key usage: {e}', webhook_url)
